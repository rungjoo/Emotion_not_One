...
from utils import make_batch_roberta, make_batch_bert
...
def main():    
    ...
    for epoch in tqdm(range(training_epochs)):
        ...
        for i_batch, data in enumerate(train_dataloader):
            ...
            """Prediction"""
            batch_input_tokens, batch_labels, batch_cls_positions = data
            batch_input_tokens, batch_labels = batch_input_tokens.cuda(), batch_labels.cuda()
            try:
                pred_logits = model(batch_input_tokens, batch_cls_positions)
            except:
                pdb.set_trace()
...
def _CalACC(model, dataloader):
    ...
    with torch.no_grad():
        for i_batch, data in enumerate(dataloader):            
            ...
            pred_logits = model(batch_input_tokens, batch_cls_positions) # (1, clsNum)
...
if __name__ == '__main__':
    ...
    parser.add_argument( "--pretrained", help = 'roberta-large or bert-large-uncased', default = 'roberta-large')
